...
import { logCreator, logDeleter } from '../services/maintenancelog.services';
...
export const deleteLog = async (
	req: Request<{}, {}, { id: number; vin: string; auth: any; mileage: number; desc: string }>,
	res: Response,
) => {
	const { id, vin, mileage, desc } = req.body;

	try {
		const log = await logDeleter(id);
	} catch (error) {
		if (error instanceof HttpStatus) {
			return res.status(error.status).json({ message: error.message });
		}
