...
import { vehicleCreater, fetchVehicles, vehicleDeleter } from '../services/carprofile.services';
import { HttpStatus } from '../utils/httpErrors/HttpStatus';

export const createVehicle = async (req: Request, res: Response): Promise<void> => {
	try {
		console.log(`body: ${req.body.VIN}`);
...
export const deleteVehicle = async (req: Request, res: Response): Promise<void> => {
	try {
		await vehicleDeleter(req.body.vehicleId);
	} catch (error) {
		res.status(404).json({ message: (error as HttpStatus).message });
	}
};
